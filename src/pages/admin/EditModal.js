...
export default function EditModal({ inscricao, onSave, onClose }) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-surface-border sticky top-0 bg-gray-950 z-10">
          ...
          <button
            onClick={onClose}
            className="text-brand-300 hover:text-white text-xl"
          >
            ✕
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              ...
              value={data.instagram_representante || ""}
              onChange={handleChange}
            />
            <Input
              label="Instagram Parc."
              name="instagram_parceiro"
              value={data.instagram_parceiro || ""}
              onChange={handleChange}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              label="CT Representante"
              name="ct_representante"
              value={data.ct_representante || ""}
              onChange={handleChange}
            />
            <Input
              label="CT Parceiro"
              name="ct_parceiro"
              value={data.ct_parceiro || ""}
              onChange={handleChange}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Select
                ...
                value={data.uniforme_representante || ""}
                onChange={handleChange}
              >
                {TAMANHOS.map((t) => (
              ...
              {data.uniforme_representante === "Personalizado" && (
                <Input
                  ...
                  value={customRepUniform}
                  onChange={(e) => setCustomRepUniform(e.target.value)}
                />
              )}
            </div>
            <div>
              ...
              {data.uniforme_parceiro === "Personalizado" && (
                <Input
                  className="mt-2"
                  placeholder="Tamanho personalizado"
                  value={customParcUniform}
                  onChange={(e) => setCustomParcUniform(e.target.value)}
                />
              )}
            </div>
          </div>
          ...
          <Select
            ...
            value={data.status_pagamento || "pendente"}
            onChange={handleChange}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex items-center gap-3 cursor-pointer text-brand-200 text-sm">
              <input
                type="checkbox"
                name="segunda_inscricao_rep"
                checked={!!data.segunda_inscricao_rep}
                onChange={handleChange}
            ...
            <label className="flex items-center gap-3 cursor-pointer text-brand-200 text-sm">
              <input
                type="checkbox"
                name="segunda_inscricao_parc"
                checked={!!data.segunda_inscricao_parc}
                onChange={handleChange}
                className="w-4 h-4 accent-brand-400"
              />
